#!/bin/bash
# YAML Validator v3.3.1 - Incremental Performance Benchmark
# Purpose: Measure incremental validation performance (2nd run with cache)
...
NUM_FILES=100
MIN_SPEEDUP=30.0  # Minimum 30x speedup required (2nd run)

# Results file
RESULTS_FILE="${RESULTS_FILE:-/tmp/benchmark_incremental_results.txt}"
BASELINE_FILE="${BASELINE_FILE:-/tmp/benchmark_baseline_results.txt}"

echo "=========================================="
echo "YAML Validator - Incremental Benchmark"
echo "=========================================="
echo
...
echo "Running 1st run (populate cache)..."
"$VALIDATOR" --incremental "$TEST_DIR"/*.yaml > /dev/null 2>&1

# Second run (use cache)
echo "Running 2nd run (incremental with cache)..."
START_TIME=$(date +%s.%N)
...
echo
echo -e "${GREEN}Incremental Results (2nd run):${NC}"
...
echo "=========================================="
echo "Summary:"
echo "  ✓ Incremental: ${DURATION}s (${SPEEDUP}x faster)"
echo "  ✓ Cache hits: ~${NUM_FILES} files"
echo "=========================================="
